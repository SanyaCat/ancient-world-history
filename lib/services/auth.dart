...
class AuthService {
  ...
  Future<AWHUser> signInWithEmailAndPassword(String email, String password) async {
    try {
  ...
  Future<AWHUser> registerWithEmailAndPassword(String email, String password) async {
    try {
  ...
  Stream<AWHUser> get currentUser {
    return _fAuth.authStateChanges().map((User user) => user != null ? AWHUser.fromFirebase(user) : null);
  }
}